function ExportJQGridDataToExcel(tableCtrl, excelFilename) {
        ...
        for (var j = 0; j < columnNames.length; j++) {
            ...
            if (cellValue == null || cellValue == 'null' || cellValue == "undefined")
                ...
                if (cellValue.indexOf("a href") > -1) {
                    ...
                    //cellValue = Date(cellValue.split('/Date(')[1].split(')/')[0]);
                    var substringedDate = cellValue.substring(6); //substringedDate= 1291548407008)/
                    var parsedIntDate = parseInt(substringedDate); //parsedIntDate= 1291548407008
                    cellValue = (new Date(parsedIntDate)).toLocaleDateString();  // parsedIntDate passed to date constructor
                }
                if (cellValue == "true")
                    cellValue = 'Yes';
                if (cellValue == "false")
    ...
    postAndRedirect("/BRQ/ExportToExcel?filename=" + excelFilename, { excelData: excelData });
}
